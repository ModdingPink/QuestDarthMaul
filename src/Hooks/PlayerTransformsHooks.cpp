...
{
	if (self->leftHandTransform == nullptr || self->rightHandTransform == nullptr || config.playStyleMode == 0) {
		PlayerTransforms_Update(self);
		return;
	}

	if (config.playStyleMode == 1) {
		//OneHandDarthMaul
		...
		//UnicornMode
		self->leftHandTransform->set_rotation(self->headWorldRot);
		self->rightHandTransform->set_rotation(self->headWorldRot);
		self->leftHandTransform->set_position(self->headWorldPos);
		...
		saberToMove->set_position(mainSaber->get_position());
		saberToMove->set_rotation(mainSaber->get_rotation());
